...
import {
  ...
  createAptos,
  createMovement,
  createAptosKeyClient,
  createMovementKeyClient,
  createAptosWalletClient,
  createMovementWalletClient,
...
const testConfigs = [
  {
    name: "Aptos",
    node: createAptos(),
    key: createAptosKeyClient(),
    wal: createAptosWalletClient(),
  ...
  },
  {
    name: "Movement",
    node: createMovement(),
    key: createMovementKeyClient(),
    wal: createMovementWalletClient(),
...
async function getAccountResourceWithRetry(
  ...
  while (attempt < maxRetries) {
    try {
      return await node.account.getAccountResource({
        accountAddress,
...
describe.each(testConfigs)(
  ...
  ({ name, node, key, wal, packageId, network }) => {
    ...
    describe("WalletClient", () => {
      it("discovers OpenRPC spec", async () => {
        expect(await wal.rpcDiscover()).toMatchObject({
          openrpc: "1.2.6",
          info: {
            title: `Shinami wallet service RPC for ${name}`,
            version: /\d+\.\d+\.\d+/,
          },
        });
      });
    ...
    describe("Shinami Wallet", () => {
      ...
      const signer1 = new ShinamiWalletSigner(
        walletId1,
        wal,
        "fake secret",
        key,
      ...
      it("creates and retrieves an initialized wallet address", async () => {
        ...
        const account = await getAccountResourceWithRetry(node, createdAddress);
      ...
      it(
        ...
        async () => {
          ...
          await expect(
            getAccountResourceWithRetry(node, createdAddress),
          ...
          const account = await getAccountResourceWithRetry(
            node,
            createdAddress,
          );
      ...
      it(
        ...
        async () => {
          ...
          const accountAuthenticator =
            await signer1.signTransaction(transaction);
          const signingMessage = node.getSigningMessage({ transaction });
      ...
      it(
        ...
        async () => {
          ...
          await getAccountResourceWithRetry(node, senderAcct);
          const receiverAcct = await signer2.getAddress(true, false);
      ...
      it(
        ...
        async () => {
          ...
          const transaction = await node.transaction.build.multiAgent({
            sender: senderAcct,
            data: {
              function: "0x1::aptos_account::transfer",
              functionArguments: [receiverAcct, 0],
