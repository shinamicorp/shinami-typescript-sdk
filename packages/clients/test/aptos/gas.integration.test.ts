...
describe.each(testConfigs)(
  "GasStationClient ($name)",
  ({ name, node, gas, packageId, network }) => {
    beforeAll(async () => {
    ...
    const happyTest =
      (options?: InputGenerateTransactionOptions) => async () => {
        ...
        expect(
          committed.events.find((x) => x.type == `${packageId}::math::Result`),
        ).toMatchObject({
          data: {
            result: "3", // 1 + 2
          },
    ...
    it("fails to sponsor and submit transaction with bad signature", async () => {
      ...
      const promise = gas.sponsorAndSubmitSignedTransaction(
        transaction,
        badSig,
      );
